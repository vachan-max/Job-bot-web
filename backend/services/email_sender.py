...
def _build_html(jobs: list, prefs: dict) -> str:
    ...
    cards = "".join(
        _build_job_card(i+1, job, show_ai, show_match, show_cover)
...
def send_email(jobs: list, to_email: str, prefs: dict = None):
    ...
    try:
        ...
        params = {
            "from": f"PingScore Job Bot <{FROM_EMAIL}>",
            "to": [to_email],
            "subject": f"🔔 PingScore — {len(jobs)} Job Alert{'s' if len(jobs) > 1 else ''} For You",
            "html": html,
        }

        response = resend.Emails.send(params)
        print(f"[email] Sent {len(jobs)} jobs to {to_email} | id: {response['id']}")

    except Exception as e:
        print(f"[email] Error: {e}")
        print(f"[email] FROM: {FROM_EMAIL}")
        print(f"[email] FROM field: PingScore Job Bot <{FROM_EMAIL}>")
...
def send_no_jobs_email(to_email: str, name: str, reason: str = ""):
    try:
        ...
        params = {
            "from": f"PingScore Job Bot <{FROM_EMAIL}>",
            "to": [to_email],
            "subject": "🔔 PingScore — No New Jobs Today",
            "html": html,
        }

        response = resend.Emails.send(params)
        print(f"[email] No-jobs notification sent to {to_email} | id: {response['id']}")
