...
// ─── tiny icon components (no emoji, pure SVG) ───────────────────────────────
const IconBriefcase = () => (
...
export default function Dashboard() {
  ...
  const [runResult, setRunResult] = useState(null); // { success, message, jobs_sent }
  ...
  async function fetchSchedulerStatus() {
    try {
      ...
      console.error("schedulerStatus error:", e);
    }
  }
  ...
  async function fetchRecentJobs() {
    try {
      ...
      // backend may return an array OR an object like { jobs: [...] } or { alerts: [...] }
      let data = res.data;
  ...
  async function handleSavePrefs(e) {
    ...
    try {
      ...
      alert("Failed to save preferences. Check the console.");
    } finally {
  ...
  async function handleRunNow() {
    try {
      ...
      const msg = err.response?.data?.detail || "Something went wrong. Check console.";
    ...
    <div style={styles.root}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input[type=range] { -webkit-appearance:none; width:100%; height:4px; border-radius:2px; background:linear-gradient(to right,#6366f1 0%,#6366f1 calc((var(--val,70)-0)/(100-0)*100%),#e2e8f0 calc((var(--val,70)-0)/(100-0)*100%),#e2e8f0 100%); }
      ...
      <nav style={styles.nav}>
        <div style={styles.navInner}>
          ...
          <div style={styles.navLinks}>
            ...
            <button style={{ ...styles.navLink, ...styles.btnLogout }} className="btn-logout" onClick={handleSignOut}>
              <IconLogout /> Sign out
            </button>
          </div>
        </div>
              ...
              <form onSubmit={handleSavePrefs} style={styles.form}>
                ...
                <FieldGroup icon={<IconBriefcase />} label="Key Skills (comma separated)" htmlFor="skills">
                  <input
                    ...
                    className="field-input"
                    style={styles.input}
                    placeholder="e.g. Python, Machine Learning, FastAPI"
                    value={prefs.skills}
                  ...
                  <FieldGroup icon={<IconStar />} label={`Min AI Score: ${prefs.min_score}`} htmlFor="min_score">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setPrefs({ ...prefs, min_score: val });
                        e.target.style.setProperty("--val", val);
                      }}
                      ref={el => {
                        if (el) el.style.setProperty("--val", prefs.min_score);
                      }}
                    />
                ...
                <div style={styles.togglesCard}>
                  <p style={styles.togglesTitle}>Pipeline Features</p>
                  <p style={styles.togglesSub}>Choose what happens when jobs are fetched</p>
                ...
                <div style={styles.formFooter}>
                  ...
                  {savedOk && <span style={styles.savedText}><IconCheck /> Preferences updated</span>}
                </div>
              </form>
            )}
          </section>

          {/* ── RIGHT: run now + recent ── */}
          <div style={styles.rightCol}>

            {/* Run Now card */}
            <section style={{ ...styles.card, ...styles.runCard }} className="card">
              <div style={styles.runIcon}>
                <IconWhatsapp />
              </div>
              <h2 style={styles.cardTitle}>Send Jobs Now</h2>
              <p style={styles.cardSub}>Manually trigger a job search and send results to your WhatsApp immediately.</p>
                  ...
                  {recentJobs.map((job, i) => (
                    <div key={i} className="job-card" style={styles.jobCard}>
                      <div style={styles.jobMeta}>
                        <span style={styles.jobTitle}>{job.job_title || "Job"}</span>
                        <span
                          className="score-pill"
                          style={{
                            background: `${scoreColor(job.ai_score)}18`,
                            color: scoreColor(job.ai_score),
                          }}
                        >
                          {job.ai_score ?? "--"}
                        </span>
                      ...
                      {job.apply_link && (
                        <a href={job.apply_link} target="_blank" rel="noreferrer" style={styles.applyLink}>
                          Apply →
                        </a>
                      )}
                    </div>
